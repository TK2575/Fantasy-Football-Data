...
DROP TRIGGER IF EXISTS `2017_ff`.`raw_df_BEFORE_INSERT`;

DELIMITER $$
USE `2017_ff`$$
CREATE DEFINER = CURRENT_USER TRIGGER `2017_ff`.`raw_df_BEFORE_INSERT` BEFORE INSERT ON `raw_df` FOR EACH ROW
BEGIN
