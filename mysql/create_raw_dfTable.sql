...
DROP TRIGGER IF EXISTS `2021_ff`.`raw_df_BEFORE_INSERT`;

DELIMITER $$
USE `2021_ff`$$
CREATE DEFINER = CURRENT_USER TRIGGER `2021_ff`.`raw_df_BEFORE_INSERT` BEFORE INSERT ON `raw_df` FOR EACH ROW
BEGIN
