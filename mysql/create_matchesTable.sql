...
DROP TRIGGER IF EXISTS `2021_ff`.`matches_BEFORE_INSERT`;

DELIMITER $$
USE `2021_ff`$$
CREATE DEFINER = CURRENT_USER TRIGGER `2021_ff`.`matches_BEFORE_INSERT` BEFORE INSERT ON `matches` FOR EACH ROW
BEGIN
