...
DROP TRIGGER IF EXISTS `2018_ff`.`matches_BEFORE_INSERT`;

DELIMITER $$
USE `2018_ff`$$
CREATE DEFINER = CURRENT_USER TRIGGER `2018_ff`.`matches_BEFORE_INSERT` BEFORE INSERT ON `matches` FOR EACH ROW
BEGIN
